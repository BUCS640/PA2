...
def scan(line: list, p1, p2):
    ...
    for i in range(len(line)):
        ...
        if cur == ' ' or (cur != ' ' and (cur == cur_check or cur_check == '\0')):
            if has_checker and len(cur_pattern) == 6:
                score, elements_id = match(str(cur_pattern), i-6)
            ...
            if has_checker and len(cur_pattern) >= 5:
                score, elements_id = match(str(cur_pattern), i-len(cur_pattern))
                store(score, elements_id)
            cur_pattern.clear()
    ...
    if has_checker and len(cur_pattern) >= 5:
        score, elements_id = match(str(cur_pattern), len(line) - len(cur_pattern))
        store(score, elements_id)
...
class AIPlayer(Player):
    ...
    def __init__(self, checker, difficulty=1):
